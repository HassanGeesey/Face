...
export function validateQuotation(quotation) {
  ...
  if (!quotation.company) {
    ...
    if (!Array.isArray(mobiles) || mobiles.length === 0) {
      errors.push("Company mobiles must be a non-empty array.");
    } else if (mobiles.some((m) => typeof m !== "string" || !m.trim())) {
      errors.push("Company mobiles must contain non-empty strings.");
    }
    if (!logo?.trim()) {
